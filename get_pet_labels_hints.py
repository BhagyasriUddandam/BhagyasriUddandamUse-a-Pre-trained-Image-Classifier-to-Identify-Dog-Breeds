...
# 
def generate_pet_labels(image_folder):
    """
    ...
    for idx in range(0, len(image_files), 1):
       
       # Skips file if starts with . (like .DS_Store of Mac OSX) because it 
       # isn't an pet image file
       if image_files[idx][0] != ".":
           
           # Creates temporary label variable to hold pet label name extracted 
           pet_label = ""

           # TODO: 2a. BELOW REPLACE pass with CODE that will process each 
           #          filename in the image_files list to extract the dog breed 
           #          name from the filename. Recall that each filename can be
           #          accessed by image_files[idx]. Be certain to place the 
           ...
           if image_files[idx] not in labels_dict:
              labels_dict[image_files[idx]] = [pet_label]
              
           else:
               print("** Warning: Duplicate files exist in directory:", 
                     image_files[idx])
 
    # TODO 2b. Replace None with the labels_dict dictionary that you created
    # with this function
    return None
