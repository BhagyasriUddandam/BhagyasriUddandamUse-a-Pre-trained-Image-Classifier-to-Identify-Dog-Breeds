...
def check_command_line_arguments(cmd_args):
    """
    For Lab: Classifying Images - 7. Command Line Arguments
    Prints each of the command line arguments passed in as parameter cmd_args, 
    assumes you defined all three command line arguments as outlined in 
    '7. Command Line Arguments'
    Parameters:
     cmd_args -data structure that stores the command line arguments object
    Returns:
    ...
    if cmd_args is None:
        ...
        print("Command Line Arguments:\n     dir =", cmd_args.dir, 
              "\n    arch =", cmd_args.arch, "\n dogfile =", cmd_args.dogfile)

def check_creating_pet_image_labels(results_dictionary):
    """    
    ...
    pairs in your results_dictionary dictionary. Assumes you defined the results_dictionary
    dictionary as was outlined in 
    '9/10. Creating Pet Image Labels'
    Parameters:
      results_dictionary - Dictionary with key as image filename and value as a List 
             (index)idx 0 = pet image label (string)
    Returns:
     Nothing - just prints to console  
    """
    if results_dictionary is None:
        ...
        stop_point = len(results_dictionary)
        if stop_point > 10:
            stop_point = 10
        print("\nPet Image Label Dictionary has", len(results_dictionary),
        ...
        for key in results_dictionary:
            ...
            if n < stop_point:
                print("{:2d} key: {:>30}  label: {:>26}".format(n+1, key,
                      results_dictionary[key][0]) )

                # Increments counter
                n += 1
            
            # If past first 10 (or fewer) labels then breaks out of the loop
            else:
                break

def check_classifying_images(results_dictionary):
    """    
    ...
    processed. Assumes you defined the results_dictionary dictionary as was 
    ...
    if results_dictionary is None:
    ...
    elif len(results_dictionary[next(iter(results_dictionary))]) < 2:
        print("* Doesn't Check the Results Dictionary because 'classify_images' hasn't been defined.")
    else:
        ...
        for key in results_dictionary:
            ...
            if results_dictionary[key][2] == 1:

                # Increments Match counter
                n_match += 1
                print("\n{:>30}: \nReal: {:>26}   Classifier: {:>30}".format(key, 
                      results_dictionary[key][0], results_dictionary[key][1]))
...
def check_classifying_labels_as_dogs(results_dictionary):
    """    
    ...
    all 40 images are processed. Assumes you defined the results_dictionary dictionary
    ...
    elif len(results_dictionary[next(iter(results_dictionary))]) < 4 :
        print("* Doesn't Check the Results Dictionary because 'adjust_results4_isadog' hasn't been defined.")
        ...
        for key in results_dictionary:
            ...
            if results_dictionary[key][2] == 1:
                ...
                print("\n{:>30}: \nReal: {:>26}   Classifier: {:>30}  \nPetLabelDog: {:1d}  ClassLabelDog: {:1d}".format(key,
                      results_dictionary[key][0], results_dictionary[key][1], results_dictionary[key][3], 
                      results_dictionary[key][4]))
...
def check_calculating_results(results_dictionary, results_stats_dictionary):
    """    
      ...
      results_dictionary - Dictionary with key as image filename and value as a List 
             (index)idx 0 = pet image label (string)
                    ...
                    idx 4 = 1/0 (int)  where 1 = Classifier classifies image 
                            'as-a' dog and 0 = Classifier classifies image  
                            'as-NOT-a' dog.
     results_stats_dictionary - Dictionary that contains the results statistics (either
    ...
    Returns:
     Nothing - just prints to console  
    """
    if results_stats_dictionary is None:
        print("* Doesn't Check the Results Dictionary because 'calculates_results_stats' hasn't been defined.")
    else:
        # Code for checking results_stats_dictionary -
        # Checks calculations of counts & percentages BY using results_dictionary
        # to re-calculate the values and then compare to the values
        # in results_stats_dictionary
    
        # Initialize counters to zero and number of images total
        n_images = len(results_dictionary)
        n_pet_dog = 0
        n_class_cdog = 0
        n_class_cnotd = 0
        n_match_breed = 0
    
        # Interates through results_dictionary dictionary to recompute the statistics
        ...
        for key in results_dictionary:
            ...
            if results_dictionary[key][2] == 1:
                ...
                if results_dictionary[key][3] == 1:
                    n_pet_dog += 1

                    # isa dog (classifier label) & breed match
                    if results_dictionary[key][4] == 1:
                        n_class_cdog += 1
                        n_match_breed += 1
        ...
        print("N Images: {:2d}  N Dog Images: {:2d}  N NotDog Images: {:2d} \nPct Corr dog: {:5.1f} Pct Corr NOTdog: {:5.1f}  Pct Corr Breed: {:5.1f}".format(
              results_stats_dictionary['n_images'], results_stats_dictionary['n_dogs_img'],
              results_stats_dictionary['n_notdogs_img'], results_stats_dictionary['pct_correct_dogs'],
              results_stats_dictionary['pct_correct_notdogs'],
              results_stats_dictionary['pct_correct_breed']))
