...
# 
def adjust_results_for_is_dog(results_dict, dog_file):
    """
    ...
    """           
    # Creates dog_names dictionary for quick matching to results_dict labels from
    # real answer & classifier's answer
    dog_names_dict = dict()

    # Reads in dog names from file, 1 name per line & automatically closes file
    with open(dog_file, "r") as infile:
        # Reads in dog names from first line in file
        line = infile.readline()

        # Processes each line in file until reaching EOF (end-of-file) by 
        # processing line and adding dog names to dog_names_dict with while loop
        while line != "":
            # Process line by striping newline from line
            dog_name = line.strip()

            # adds dog_name to dog_names_dict if it doesn't already exist 
            # in the dog_names_dict dictionary
            if dog_name not in dog_names_dict:
                dog_names_dict[dog_name] = 1
    ...
    # How - iterate through results_dict if labels are found in dog_names_dict
    # then label "is a dog" index3/4=1 otherwise index3/4=0 "not a dog"
    for key in results_dict:
        # Pet Image Label IS of Dog (e.g. found in dog_names_dict)
        if results_dict[key][0] in dog_names_dict:
            # Classifier Label IS image of Dog (e.g. found in dog_names_dict)
            # appends (1, 1) because both labels are dogs
            if results_dict[key][1] in dog_names_dict:
                results_dict[key].extend((1, 1))
            # Classifier Label IS NOT image of dog (e.g. NOT in dog_names_dict)
            # appends (1, 0) because only pet label is a dog
            else:
                results_dict[key].extend((1, 0))
        # Pet Image Label IS NOT a Dog image (e.g. NOT found in dog_names_dict)
        else:
            # Classifier Label IS image of Dog (e.g. found in dog_names_dict)
